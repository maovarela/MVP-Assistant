...
- **FIX crítico: montos BNP ≥ €1.000 venían invertidos** (`bankCsv.reverseFrenchAmount`): el PDF de BNP alinea los números por columna y emite la parte entera partida en segmentos de whitespace en orden **invertido** (innermost-digit-first), igual que los headers SOLDE. El parser de transacciones solo quitaba espacios y concatenaba en orden directo → `972<tab>1` daba 9721 en vez de 1972. Resultado: salario, renta y transferencias grandes guardadas al revés en TODO el histórico. Fix: invertir los segmentos antes de concatenar (igual que `parseReversedBnpAmount` ya hacía para balances). Verificado: la renta de mayo pasa de €9.721,81 → **€1.972,81**, salario €7.213,70 → **€3.721,70**, y el statement reconcilia exacto (opening 5412,47 + movimientos = closing 4147,32). **Pendiente**: limpiar + re-importar el histórico de BNP (las filas viejas tienen el monto invertido; re-importar no las corrige porque el hash cambia → hay que borrar las `bnp:%` y re-subir).
